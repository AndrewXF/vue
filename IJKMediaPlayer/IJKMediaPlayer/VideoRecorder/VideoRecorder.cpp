...
//        c = avcodec_alloc_context3(video_codec);
    ...
    void VideoRecorderImpl::open_video() {
        ...
        if (avpicture_alloc(&dst_picture, videoOpenContext->pix_fmt, video_srcHight, video_height) < 0) {
        ...
        if (videoOpenContext->pix_fmt != AV_PIX_FMT_YUV420P) {
            if (avpicture_alloc(&src_picture, AV_PIX_FMT_YUV420P, video_srcHight, video_height) < 0) {
        ...
        *((AVPicture *) picture) = dst_picture;
        ...
        img_convert_ctx = sws_getContext(video_srcHight, video_height, video_pixfmt, video_srcHight,
                                         videoOpenContext->height, AV_PIX_FMT_YUV420P, /*SWS_BICUBIC*/
    ...
    void VideoRecorderImpl::SupplyVideoFrame(const void *frameData, unsigned long timestamp) {
        ...
        if (video_pixfmt != AV_PIX_FMT_YUV420P) {
            ...
            sws_scale(img_convert_ctx, src_picture.data, src_picture.linesize, 0,
                      video_height, dst_picture.data, dst_picture.linesize);
        } else {
            av_picture_copy(&dst_picture, &src_picture, video_pixfmt, video_srcHight, video_height);
        }
        /**
